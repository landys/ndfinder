...
int calc_grad_mag_ori( IplImage*, int, int, double*, double*, double*, double* );
...
{
	double* hist;
	double mag, ori, w, exp_denom, PI2 = CV_PI * 2.0, x, y;
	int bin, i, j;
	...
	for( i = -rad; i <= rad; i++ )
		for( j = -rad; j <= rad; j++ )
			if( calc_grad_mag_ori( img, r + i, c + j, &mag, &ori, &x, &y ) )
			{
...
*/
int calc_grad_mag_ori( IplImage* img, int r, int c, double* mag, double* ori, double* x, double* y )
{
	double dx, dy;
	double i, j;
...
{
	double*** hist;
	double grad_mag,grad_ori,x,y;
	int radius = 19, i, j;

	if( histtest == NULL){
		histtest = (double***) calloc( 39, sizeof( double** ) );
		for( i = 0; i < 39; i++ )
		{
			histtest[i] = (double**) calloc( 39, sizeof( double* ) );
			for( j = 0; j < 39; j++ )
				histtest[i][j] = (double*) calloc( 2, sizeof( double ) );
		}
	}
	...
	for( i = -radius; i <= radius; i++ )
		for( j = -radius; j <= radius; j++ )
		{
			...
			if( calc_grad_mag_ori( img, r + i, c + j, &grad_mag, &grad_ori,&x,&y ))
			{
				//	printf("%f   %f",grad_mag,grad_ori);
				//	getchar();
				hist[i+radius][j+radius][0] = x;
				hist[i+radius][j+radius][1] = y;
			}
		}
...
void hist_to_descr( double*** hist, int d, int n, struct feature* feat )
{
	...
	int ldi, x;
	int ii,jj;
	//	printf( "%f %f %f %f\n", feat->y, feat->x,feat->scl, feat->ori );
	for( r = 0; r < 39; r++ )
		for( c = 0; c < 39; c++ )
			for( o = 0; o < 2; o++ ){
				feat->descr[k++] = hist[r][c][o];
				total +=  fabs(hist[r][c][o]);
			}
			total /= len;

			//	for (i = 0; i < len; i++) {
			//		feat->descr[i] = feat->descr[i] / total * 100.0;
			//	}

			for (ldi = 0; ldi < PCASIZE; ldi++) {
				feat->PCAdescr[ldi] = 0;
				for (x = 0; x < FEATURE_MAX_D; x++)
					feat->PCAdescr[ldi] += eigs[ldi][x] * feat->descr[x];
			}
